...
Type
  States=(move_start,cube_opers,from_stakan,from_slot,wait); //тип состояний игры
  TPlayer=record   //тип игрока
    Name:string[20];    //имя
    One:string[2];      //комбинации
    Two:string[2];
    Three:string[2];
    Four:string[2];
    Five:string[2];
...
var
  Form1: TForm1;
  Slot:array[0..4] of 0..6; //массив слотов, в который записывается, какой кубик находится внутри каждого отдельного слота
  Stakan:array[0..4] of 0..6; //массив слотов стакана, в который записывается, какой кубик находится внутри каждого отдельного слота
  Cubes:array[0..4] of 0..6; //массив кубиков из стакана и слотов (для удобства заполнения таблицы)
  Players:array[1..8] of TPlayer; //массив игроков
  State:States; //текущее состояние игры
  ChosenCube:0..6; //выбранный кубик
  ChosenPos:byte; //выбранная позиция
  CurrentPlayer:byte; //текущий игрок
  NextPlayer:byte; //следующий игрок
  CurrentMove:byte; //номер текущего хода
  CurrentThrow:byte; //номер текущего броска
  PlayersCount:byte; //количество игроков
  CubesCount:byte; //количество кубиков в стакане
  CellChosen:byte; //выбранная ячейка
...
begin
  ...
  label1.Caption:='Текущий игрок: '+Players[1].Name;
  label2.Caption:='Следующий игрок: '+Players[2].Name;
  State:=move_start;
  CurrentThrow:=0;
  CurrentMove:=1;
  //CurrentMove:=15;
  ...
  for i:=0 to 4 do
    Slot[i]:=0;
  CubesCount:=5;
  Label4.Caption:='Ход игрока '+Players[1].Name+'. Нажмите на стакан, чтобы выбросить кости.';
  Image2.Visible:=true;
  Button1.Enabled:=false;
end;

procedure SlotStakanToCube;             //складывание костей из стакана
var i,j:integer;                        //и из слотов
begin
j:=-1;
  for i:=0 to 4 do
    begin
...
////////////////////////////////////////////////////////////////////////////////
procedure CombTableEdit;                  //обработка таблицы комбинаций
var i,j,Sum:integer;
cas:boolean;
...
begin
  ...
  for j:=1 to 6 do
    CubeCount[j]:=0;
  //работа с комбинацией "короткий стрит"
  for j:=1 to 6 do
    CubeCount[j]:=0;
  ...
  for j:=1 to 6 do
    CubeCount[j]:=0;
  cas:=false;
  //работа с комбинацией "длинный стрит"
  for j:=1 to 6 do
    CubeCount[j]:=0;
...
////////////////////////////////////////////////////////////////////////////////
procedure GetChosenCube;                          //выбранная кость
begin
  if (State=from_stakan)or(State=from_slot) then
    begin
      ...
      Form1.Label4.Caption:='Нажмите на любую ячейку, чтобы положить кость '+inttostr(ChosenCube)+'.';
      Form1.Memo1.Lines.Add('Игрок '+Players[CurrentPlayer].Name+' взял кость '+inttostr(ChosenCube)+'.');
    end;
...
////////////////////////////////////////////////////////////////////////////////
procedure FillWithChosenCube;                      //заполнение ячеек
begin
...
////////////////////////////////////////////////////////////////////////////////
procedure FillWithChosenPos;                  //заполнение ячеек
begin
  if(ChosenPos=1) then
    begin
      if ChosenCube=1 then Form1.DrawGrid2.Canvas.Draw(0,0,Form1.Image1.Picture.Graphic);
...
procedure TForm1.DrawGrid1SelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);                  //обработка перемещения кости
...
begin
  if (State=from_stakan)and(Slot[ARow]=0) then
    begin
      ...
      GetChosenCube;
      if CurrentThrow=3
        then Label4.Caption:='Запишите комбинацию.' else
          Label4.Caption:='Запишите комбинацию, переместите кубик или нажмите на стакан, чтобы перемешать и выбросить кости.';
      Memo1.Lines.Add('Игрок '+Players[CurrentPlayer].Name+' положил кость '+inttostr(ChosenCube)+' в слот.');
      ChosenCube:=0;
    end else
      if (State=from_stakan)and(Slot[ARow]<>0) then
        begin
...
////////////////////////////////////////////////////////////////////////////////
procedure randomCubes;                        //генерация костей
var i,Cube:integer;
begin
...
////////////////////////////////////////////////////////////////////////////////
procedure TForm1.Image2Click(Sender: TObject);  //бросок костей
var i:integer;
begin
  if (State=move_start) then
    begin
      State:=cube_opers;
      Label4.Caption:='Запишите комбинацию, переместите кубик или нажмите на стакан, чтобы перемешать и выбросить кости.';
      Memo1.Lines.Add('Игрок '+Players[CurrentPlayer].Name+' совершает первый бросок.');
      CurrentThrow:=1;
      Label3.Caption:='Бросок: 1/3';
      randomCubes;
      CombTableEdit;
    end else
      if (CurrentThrow=1)and(State=cube_opers) then
        begin
        Label3.Caption:='Бросок: 2/3';
        Memo1.Lines.Add('Игрок '+Players[CurrentPlayer].Name+' делает второй бросок.');
        inc(CurrentThrow);
        for i:=0 to 4 do
          Stakan[i]:=0;
        ...
        CombTableEdit;
          ...
          if (CurrentThrow=2)and(State=cube_opers) then
            begin
            inc(CurrentThrow);
            Label4.Caption:='Запишите комбинацию.';
            Memo1.Lines.Add('Игрок '+Players[CurrentPlayer].Name+' делает третий и последний бросок.');
            for i:=0 to 4 do
              Stakan[i]:=0;
            DrawGrid2.Canvas.FillRect(Rect(0,0,50,50));
            DrawGrid3.Canvas.FillRect(Rect(0,0,50,50));
            ...
            //Button1.Enabled:=true;
              ...
              if (CurrentThrow=3)and(State=cube_opers) then
                begin
                  MessageDlg('Вы не можете сделать бросок! Выберите комбинацию.',mtWarning,[mbOK],0);
                end else
                  if (State=from_stakan)or(State=from_slot) then
                    begin
                      MessageDlg('Вы не можете сделать бросок! Положите в свободную ячейку кубик с ребром '+inttostr(ChosenCube)+'.',mtWarning,[mbOK],0);  
                    end;
end;
////////////////////////////////////////////////////////////////////////////////
procedure TForm1.Button1Click(Sender: TObject);    //передача хода
var i:integer;
begin
  ...
  Label3.Caption:='Бросок: 0/3';
  Label4.Caption:='Ход игрока '+Players[CurrentPlayer].Name+'. Нажмите на стакан, чтобы выбросить кости.';
  Memo1.Lines.Add('Ход игрока '+Players[CurrentPlayer].Name+'.');
  State:=move_start;
  ChosenCube:=0;
  Button1.Enabled:=false;
  ...
  label1.Caption:='Текущий игрок: '+Players[CurrentPlayer].Name;
  label2.Caption:='Следующий игрок: '+Players[NextPlayer].Name;
  for i:=0 to 4 do
    begin
      Stakan[i]:=0;
      Slot[i]:=0;
      ...
      CubesCount:=CubesCount-1;
          ...
          if(State=from_stakan)and(ChosenPos<>1)and(Stakan[0]<>0) then
            begin
              ...
              TempCube:=ChosenCube;
              FillWithChosenCube;
              Memo1.Lines.Add('Игрок '+Players[CurrentPlayer].Name+' положил кость '+inttostr(ChosenCube)+' в стакан.');
              ChosenCube:=Stakan[0];
              GetChosenCube;
              if CurrentThrow=3
                then Label4.Caption:='Запишите комбинацию.' else
...
begin
  FormCreate(Self);
  Memo1.Lines.Add('Игра начинается!');
  Memo1.Lines.Add('Ход игрока '+Players[1].Name+'.');
end;

procedure newgame;       //начало новой игры
var i:integer;
s:string;
begin
  s:='Результаты:';
  for i:=1 to PlayersCount do
    begin
      s:=s+#13#10+Players[i].Name+': '+Form2.StringGrid1.Cells[i,16]+' очков.';
    end;
...
begin
  if MessageDlg('Вы действительно хотите начать новую игру?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
    newgame;
end;
...
procedure TForm1.DrawGrid2DrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);      //прорисовка ячейки
begin
